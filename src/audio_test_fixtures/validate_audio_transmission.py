...
def analyze_frequencies(reference_audio: np.ndarray, decoded_audio: np.ndarray, 
                       sample_rate: int, tolerance_hz: float, verbose: bool = False) -> List[FrequencyMatch]:
    """
    ...
    if verbose:
        ...
        print(f"Ventana de análisis: {TONE_DURATION:.3f}s por nota")
        print()
        print("Nota  Esperada   Detectada  Error(Hz)  Error(%)  Estado")
        print("-" * 60)
    
    for i, (note_name, expected_freq) in enumerate(VOCAL_NOTES):
        # Calcular posición temporal del segmento
        segment_start_time = i * NOTE_DURATION
        segment_center_time = segment_start_time + TONE_DURATION / 2
        
        # Extraer segmento de audio (centrado en el tono, evitando silencios)
        start_sample = int((segment_start_time + 0.01) * sample_rate)  # Skip 10ms inicial
        duration_samples = int((TONE_DURATION - 0.02) * sample_rate)  # Análisis en el centro del tono
        ...
        segment = decoded_audio[start_sample:end_sample]
...
def validate_transmission(reference_path: str, decoded_path: str, 
                         tolerance_hz: float = 5.0, verbose: bool = False) -> ValidationResult:
    """
    ...
    try:
        ...
        frequency_matches = analyze_frequencies(ref_audio, dec_audio, dec_sr, tolerance_hz, verbose)
        
        # 7. Calcular métricas
        frequency_accuracy, mean_freq_error_hz = calculate_metrics(frequency_matches, snr_db, verbose)
        
        # 8. Determinar si pasó la validación
        passed = (
            metadata_match and
            duration_match and
